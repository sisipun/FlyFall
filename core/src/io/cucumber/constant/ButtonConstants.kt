package io.cucumber.constant

object ButtonConstants {

    const val START_GAME_BUTTON_WIDTH = 220F
    const val START_GAME_BUTTON_HEIGHT = 80F

    const val SOUND_OFF_BUTTON_WIDTH = 80F
    const val SOUND_OFF_BUTTON_HEIGHT = 80F

    const val HOME_BUTTON_WIDTH = 80F
    const val HOME_BUTTON_HEIGHT = 80F

    const val PAUSE_BUTTON_WIDTH = 40F
    const val PAUSE_BUTTON_HEIGHT = 40F

    const val RESUME_BUTTON_WIDTH = 220F
    const val RESUME_BUTTON_HEIGHT = 80F

}